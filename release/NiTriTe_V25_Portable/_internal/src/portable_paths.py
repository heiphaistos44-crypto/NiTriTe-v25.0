...
def get_portable_temp_dir(subfolder: str = None) -> Path:
    """
    Obtenir le dossier temporaire portable

    Crée un dossier 'temp' à côté de l'exe pour stocker :
    ...
    Args:
        subfolder: Sous-dossier optionnel (ex: 'downloads', 'scripts', 'benchmark')

    Returns:
        Path: Chemin absolu du dossier temporaire
    """
    exe_dir = get_executable_dir()

    if subfolder:
        temp_dir = exe_dir / 'temp' / subfolder
    else:
        temp_dir = exe_dir / 'temp'

    try:
        temp_dir.mkdir(parents=True, exist_ok=True)
        logger.debug(f"Dossier temp portable: {temp_dir}")
    except Exception as e:
        logger.warning(f"Impossible de créer {temp_dir}: {e}")
        # Fallback: utiliser le temp système
        import tempfile
        if subfolder:
            temp_dir = Path(tempfile.gettempdir()) / 'nitrite_temp' / subfolder
        else:
            temp_dir = Path(tempfile.gettempdir()) / 'nitrite_temp'
        temp_dir.mkdir(parents=True, exist_ok=True)
        logger.warning(f"Utilisation du fallback: {temp_dir}")

    return temp_dir


def get_portable_backups_dir() -> Path:
    """
    Obtenir le dossier backups portable

    Crée un dossier 'backups' à côté de l'exe pour stocker les sauvegardes

    Returns:
        Path: Chemin du dossier backups portable
    """
    exe_dir = get_executable_dir()
    ...
    try:
        ...
        logger.debug(f"Dossier backups portable: {backups_dir}")
    except Exception as e:
        logger.warning(f"Impossible de créer {backups_dir}: {e}")
        # Fallback: créer dans temp
        backups_dir = exe_dir / 'temp' / 'backups'
        backups_dir.mkdir(parents=True, exist_ok=True)
        logger.warning(f"Utilisation du fallback: {backups_dir}")

    return backups_dir


def get_portable_downloads_dir() -> Path:
    """
    Obtenir le dossier downloads portable pour PortableApps, etc.

    Crée un dossier 'downloads' à côté de l'exe

    Returns:
        Path: Chemin du dossier downloads portable
    """
    exe_dir = get_executable_dir()
    ...
    try:
        ...
        logger.debug(f"Dossier downloads portable: {downloads_dir}")
    except Exception as e:
        logger.warning(f"Impossible de créer {downloads_dir}: {e}")
        # Fallback: utiliser temp/downloads
        downloads_dir = exe_dir / 'temp' / 'downloads'
        downloads_dir.mkdir(parents=True, exist_ok=True)
        logger.warning(f"Utilisation du fallback: {downloads_dir}")
...
def get_portable_info() -> dict:
    ...
    info = {
        ...
        'temp_dir': str(get_portable_temp_dir()),
    }
