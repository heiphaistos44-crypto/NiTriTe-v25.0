...
def resource_path(relative_path):
    """Retourne le chemin absolu compatible PyInstaller pour les fichiers de données."""
    try:
...
class ThemeManager:
    ...
    def save_theme_preference(theme_id):
        ...
        try:
            config_path = resource_path(os.path.join('data', 'theme_config.json'))
...
class SettingsPage(ctk.CTkFrame):
    ...
    def _create_theme_section(self, parent):
        ...
        section = ctk.CTkFrame(parent, fg_color=ModernColors.BG_CARD)
        section.pack(fill=tk.X, pady=20)
        ...
        # Grille de thèmes
        themes_grid = ctk.CTkFrame(section, fg_color=ModernColors.BG_CARD)
        themes_grid.pack(fill=tk.X, pady=20)
    ...
    def _create_theme_card(self, parent, theme_id, theme_data, is_active):
        ...
        card = ctk.CTkFrame(parent, fg_color=ModernColors.BG_LIGHT)
        ...
        # Aperçu des couleurs
        colors_frame = ctk.CTkFrame(card, fg_color=ModernColors.BG_LIGHT)
        ...
        for color in color_preview:
            color_box = ctk.CTkFrame(colors_frame, fg_color=color, width=40, height=40)
    ...
    def _create_preferences_section(self, parent):
        ...
        # Options
        options_frame = ctk.CTkFrame(section, fg_color=ModernColors.BG_CARD)
        options_frame.pack(fill=tk.X, pady=20)

        # === LANGUE ===
        lang_container = ctk.CTkFrame(options_frame, fg_color=ModernColors.BG_LIGHT)
        ...
        lang_buttons = ctk.CTkFrame(lang_container, fg_color=ModernColors.BG_LIGHT)
        lang_buttons.pack(fill=tk.X, pady=10)
        ...
        btn_fr.pack(side=tk.LEFT, padx=10)
        ...
        btn_en = ctk.CTkButton(
            ...
            text=" English",
            font=("Segoe UI", 10, "bold"),
            fg_color=ModernColors.BLUE_INFO,
            text_color=ModernColors.TEXT_PRIMARY,
            cursor="hand2",
        ...
        # === APPARENCE ===
        appearance_container = ctk.CTkFrame(options_frame, fg_color=ModernColors.BG_LIGHT)
        appearance_container.pack(fill=tk.X)
        ...
        appearance_buttons = ctk.CTkFrame(appearance_container, fg_color=ModernColors.BG_LIGHT)
        appearance_buttons.pack(fill=tk.X, pady=10)
        ...
        btn_dark.pack(side=tk.LEFT, padx=10)

        # Bouton Mode Clair
        btn_light = ctk.CTkButton(
            appearance_buttons,
            text=" Mode Clair / Light Mode",
            font=("Segoe UI", 10, "bold"),
            fg_color="#f5f5f5",
            text_color="#000000",
            cursor="hand2",
    ...
    def _save_language_preference(self, lang_code):
        ...
        try:
            config_path = resource_path(os.path.join('data', 'language_config.json'))
...
class DiagnosticPage(ctk.CTkFrame):
    ...
    def _update_performance_realtime(self):
        ...
        try:
            ...
            if self.perf_widgets['cpu_percent']:
                self.perf_widgets['cpu_percent'].configure(text=f"{cpu_percent:.1f}%")
            ...
            if PSUTIL_AVAILABLE:
                ...
                if self.perf_widgets['ram_percent']:
                    self.perf_widgets['ram_percent'].configure(text=f"{ram.percent:.1f}%")
            ...
            if PSUTIL_AVAILABLE:
                ...
                if self.perf_widgets['disk_percent']:
                    self.perf_widgets['disk_percent'].configure(text=f"{disk.percent:.1f}%")
    ...
    def _create_health_score(self):
        ...
        card = ctk.CTkFrame(self.scrollable_frame, fg_color=ModernColors.BG_CARD)
        ...
        score_frame = ctk.CTkFrame(card, fg_color=ModernColors.BG_CARD)
        ...
        # Détails
        details_frame = ctk.CTkFrame(score_frame, fg_color=ModernColors.BG_CARD)
    ...
    def _create_system_info(self):
        ...
        info_frame = ctk.CTkFrame(card, fg_color=ModernColors.BG_CARD)
        ...
        for label, value in infos:
            row = ctk.CTkFrame(info_frame, fg_color=ModernColors.BG_LIGHT)
            row.pack(fill=tk.X)
    ...
    def _create_performance_section(self):
        ...
        perf_frame = ctk.CTkFrame(card, fg_color=ModernColors.BG_CARD)
    ...
    def _create_performance_bar(self, parent, label, percent, color):
        ...
        container = ctk.CTkFrame(parent, fg_color=ModernColors.BG_CARD)
        container.pack(fill=tk.X)

        # Label et pourcentage
        top_row = ctk.CTkFrame(container, fg_color=ModernColors.BG_CARD)
        ...
        bar_bg = ctk.CTkFrame(container, fg_color=ModernColors.BG_LIGHT, height=20)
    ...
    def _create_benchmark_section(self):
        ...
        header = ctk.CTkLabel(
            ...
            font=("Segoe UI", 16, "bold"),
            fg_color=ModernColors.BG_CARD,
            text_color=ModernColors.YELLOW_WARNING,
            anchor='w')
        header.pack(fill=tk.X)

        buttons_frame = ctk.CTkFrame(card, fg_color=ModernColors.BG_CARD)
        ...
        for name, desc, command, color in benchmarks:
            btn_frame = ctk.CTkFrame(buttons_frame, fg_color=ModernColors.BG_LIGHT)
        ...
        try:
            ...
            def run_benchmark():
                ...
                while time.time() - start_time < target_duration:
                    ...
                    progress_label.configure(text=f"Progression: {int((elapsed/target_duration)*100)}%")
                ...
                messagebox.showinfo(
                    ...
                    f"{' Excellent' if score > 500000 else ' Bon' if score > 250000 else ' Moyen'}"
                )
        ...
        try:
            ...
            def run_benchmark():
                ...
                for i in range(test_size):
                    ...
                    # Update progress
                    progress_label.configure(text=f"Progression: {int((i/test_size)*100)}%")
                    progress_window.update()
                ...
                messagebox.showinfo(
                    ...
                    f"{' Rapide' if speed > 1000 else ' Normal' if speed > 500 else ' Lent'}"
                )
    ...
    def _benchmark_disk(self):
        ...
        try:
            ...
            def run_benchmark():
                ...
                try:
                    # Test écriture
                    progress_label.configure(text="Test d'écriture...")
                    progress_window.update()
                    ...
                    with open(test_file, 'wb') as f:
                        for i in range(test_size):
                            f.write(data)
                            progress_label.configure(text=f"Écriture: {int((i/test_size)*100)}%")
                            progress_window.update()
                    end_write = time.time()
                    write_speed = test_size / (end_write - start_write)

                    # Test lecture
                    progress_label.configure(text="Test de lecture...")
                    progress_window.update()
                    ...
                    with open(test_file, 'rb') as f:
                        ...
                        while f.read(chunk_size):
                            i += 1
                            progress_label.configure(text=f"Lecture: {int((i/test_size)*100)}%")
                            progress_window.update()
                    end_read = time.time()
...
class BackupPage(ctk.CTkFrame):
    ...
    def _create_restore_point_section(self, parent):
        ...
        btn_frame = ctk.CTkFrame(card, fg_color=ModernColors.BG_CARD)
        btn_frame.pack(fill=tk.X, pady=20)

        create_btn = ctk.CTkButton(
            btn_frame,
            text=" Créer Point de Restauration",
            font=("Segoe UI", 11, "bold"),
            fg_color=ModernColors.GREEN_SUCCESS,
            text_color=ModernColors.TEXT_PRIMARY,
            cursor="hand2",
    ...
    def _create_driver_backup_section(self, parent):
        ...
        backup_btn = ctk.CTkButton(
            ...
            font=("Segoe UI", 11, "bold"),
            fg_color=ModernColors.PURPLE_PREMIUM,
            text_color=ModernColors.TEXT_PRIMARY,
            cursor="hand2",
    ...
    def _import_app_list(self):
        ...
        try:
            ...
            log_text.pack(padx=20, fill=tk.BOTH, expand=True)
            ...
            def install_apps():
                ...
                for i, app_name in enumerate(apps, 1):
                    status_label.configure(text=f"Installation {i}/{len(apps)}: {app_name}")
                    ...
                    try:
                        ...
                        log_text.insert(tk.END, f"⏱ Timeout (>5 min)\n", "error")
                        failed_count += 1
                        failed_apps.append(app_name)
                    except Exception as e:
                ...
                status_label.configure(text="Installation terminée !")
...
class OptimizationsPage(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        warning = ctk.CTkLabel(
            header,
            text=" Modifications système - Utiliser avec prudence",
            font=("Segoe UI", 9),
            fg_color=ModernColors.YELLOW_WARNING,
            text_color=ModernColors.BG_DARK)
    ...
    def _create_telemetry_section(self, parent):
        ...
        header = ctk.CTkLabel(
            ...
            font=("Segoe UI", 16, "bold"),
            fg_color=ModernColors.BG_CARD,
            text_color=ModernColors.RED_ERROR,
            anchor='w')
        header.pack(fill=tk.X)
        ...
        options_frame = ctk.CTkFrame(card, fg_color=ModernColors.BG_CARD)
        options_frame.pack(fill=tk.X, pady=20)
    ...
    def _create_registry_section(self, parent):
        ...
        desc = ctk.CTkLabel(
            card,
            text="Nettoyer les entrées obsolètes du registre ( Fonction avancée)",
            font=("Segoe UI", 10),
            fg_color=ModernColors.BG_CARD,
            text_color=ModernColors.TEXT_SECONDARY,
            anchor='w')
...
"""
    ...
    def _optimize_services(self):
        ...
        try:
            ...
            services_window.configure(fg_color=ModernColors.BG_DARK)
            services_window.transient(self)

            # Header
            header = ctk.CTkLabel(
                services_window,
                text=" Services Windows à optimiser",
                font=("Segoe UI", 14, "bold"),
                fg_color=ModernColors.BG_DARK,
                text_color=ModernColors.TEXT_PRIMARY
            )
            ...
            # Zone de scroll
            scroll_frame = ctk.CTkFrame(services_window, fg_color=ModernColors.BG_DARK)
            scroll_frame.pack(fill=tk.BOTH, expand=True)

            canvas = tk.Canvas(scroll_frame, bg=ModernColors.BG_CARD, highlightthickness=0)
            scrollbar = ttk.Scrollbar(scroll_frame, orient=tk.VERTICAL, command=canvas.yview)

            services_list_frame = ctk.CTkFrame(canvas, fg_color=ModernColors.BG_CARD)
            ...
            for service_name, service_desc in services_to_optimize:
                ...
                frame = ctk.CTkFrame(services_list_frame, fg_color=ModernColors.BG_LIGHT)
            ...
            # Boutons
            btn_frame = ctk.CTkFrame(services_window, fg_color=ModernColors.BG_DARK)
    ...
    def _auto_cleanup(self):
        ...
        try:
            ...
            def cleanup_task():
                ...
                try:
                    # 1. Nettoyer %TEMP%
                    status_label.configure(text="Nettoyage dossier TEMP...")
                    ...
                    status_label.configure(text="Vidage de la corbeille...")
                    log_text.insert(tk.END, " Vidage de la corbeille...\n")
                    progress_window.update()
                    ...
                    status_label.configure(text="Nettoyage cache Windows Update...")
                    ...
                    status_label.configure(text="Nettoyage fichiers journaux...")
                    ...
                    status_label.configure(text="Lancement nettoyage de disque Windows...")
                    ...
                    status_label.configure(text="Nettoyage terminé !")
                    ...
                    log_text.insert(tk.END, f"\n Erreur: {str(e)}\n")
                    status_label.configure(text="Erreur lors du nettoyage")
...
class UpdatesPage(ctk.CTkFrame):
    ...
    def _create_updates_section(self):
        ...
        update_all_btn.pack(side=tk.LEFT)
    ...
    def _check_updates(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                window.configure(fg_color=ModernColors.BG_DARK)
            ...
            def run_updates():
                try:
                    ...
                    if process.returncode == 0:
                        ...
                        log_text.insert(tk.END, f"\n Terminé avec code: {process.returncode}\n")
                        status_label.configure(text=" Terminé avec avertissements")
                    ...
                    log_text.insert(tk.END, f"\n Erreur: {str(e)}\n")
                    status_label.configure(text=" Erreur lors de la mise à jour")
